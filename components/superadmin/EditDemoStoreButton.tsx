...
export default function EditDemoStoreButton({ variant = 'header' }: Props) {
      ...
      <div className={`${superadminCard} flex flex-col sm:flex-row items-stretch sm:items-center gap-3 bg-warm/5 border-warm/30`}>
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-muted mt-1 break-words">
            Edite produtos, aparência e conteúdo da Urban Mix — abre o painel da loja (você continua logado como superadmin).
          </p>
        ...
        <div className="flex flex-col sm:flex-row gap-2 shrink-0">
          ...
          >
            ...
            {loading ? 'Abrindo…' : 'Abrir painel Urban Mix'}
          </button>
        </div>
      </div>
    )
  ...
  return (
    <div className="flex items-center gap-2 shrink-0">
      ...
      >
        <Pencil size={14} aria-hidden />
        <span className="hidden sm:inline">{loading ? 'Abrindo…' : 'Abrir painel'}</span>
