...
export default function LojaHeaderActions({ whatsapp, cartQty, onOpenCart, compact = false }: Props) {
  return (
    <div className={`flex items-center shrink-0 ${compact ? 'gap-2' : 'gap-2.5'}`}>
      <a
        ...
        className={[
          'flex items-center justify-center gap-1.5 rounded-full text-xs font-medium transition-all min-h-[44px]',
          compact ? 'min-w-[44px] px-2.5' : 'min-w-[44px] sm:min-w-0 px-3 sm:px-4 py-2',
        ].join(' ')}
        style={{
      ...
      >
        ...
        {!compact && <span className="hidden sm:inline">Vendedor</span>}
      </a>
      <button
        type="button"
        onClick={onOpenCart}
